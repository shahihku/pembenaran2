...
user=$(grep -E "^#### " "/etc/xray/vless-ws-opok.json" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^#### " "/etc/xray/vless-ws-opok.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
user=$(grep -E "^#### " "/etc/xray/vless-ws-habis.json" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^#### " "/etc/xray/vless-ws-habis.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
#sed -i "/^#### $user $exp/,/^},{/d" /etc/xray/trojan-tcp.json
sed -i "/^#### $user $exp/,/^},{/d" /etc/xray/vless-ws.json
sed -i "/^#### $user $exp/,/^},{/d" /etc/xray/vless-ws-opok.json
sed -i "/^#### $user $exp/,/^},{/d" /etc/xray/vless-ws-habis.json
